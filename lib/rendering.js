...
function Rendering(noa, _opts, canvas) {
    ...
    this.vrMode = opts.vrMode
    this.vrOffset = opts.vrOffset
    this.vrTracking = opts.vrTracking
    this.vrScaleFactor = opts.vrScaleFactor
    this.virtualJoystick = opts.virtualJoystick
...
function initScene(self, canvas, opts) {
    ...
    if(!self.vrMode && !self.virtualJoystick) {
        self._camera = new BABYLON.FreeCamera('camera', new vec3(0, 0, 0), scene)
    } else if(self.vrMode && !self.virtualJoystick) {
        //self._camera = new BABYLON.WebVRFreeCamera("camera", new BABYLON.Vector3(0, self.vrOffset, 0), scene, true, { trackPosition: self.vrTracking });
        //self._camera.deviceScaleFactor = self.vrScaleFactor;
        self._camera = new BABYLON.FreeCamera('camera', new vec3(0, 0, 0), scene)
        
        //self._camera = new BABYLON.DeviceOrientationCamera('camera', new vec3(0, 0, 0), scene);
        self._camera.setCameraRigMode(BABYLON.Camera.RIG_MODE_STEREOSCOPIC_SIDEBYSIDE_PARALLEL, {interaxialDistance: 0.0637});
    } else if(self.virtualJoystick){
        self._camera = new BABYLON.VirtualJoysticksCamera("camera", new vec3(0, 0, 0), scene);
    }
