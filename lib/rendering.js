...
function Rendering(noa, _opts, canvas) {
    ...
    this.mobileMode = opts.mobileMode
    this.gyroEnabled = opts.gyroEnabled
    this.virtualJoystick = opts.virtualJoystick
    this.enableVRDisplays = opts.enableVRDisplays
...
function initScene(self, canvas, opts) {
    ...
    if(self.vrMode && !self.virtualJoystick && self.gyroEnabled && self.mobileMode) {
        if (navigator.getVRDisplays && self.enableVRDisplays) {
            self._camera = new BABYLON.WebVRFreeCamera("camera", new vec3(0, 0, 0), scene);
        }
        else {
            rigCameras = true
            self._camera = new BABYLON.VRDeviceOrientationFreeCamera("camera", new vec3(0, 0, 0), scene);
        }
    ...
    if(!rigCameras) {
        ...
        self._camera._rigCameras[0].parent = self._cameraHolder
        self._camera._rigCameras[1].parent = self._cameraHolder
    }
