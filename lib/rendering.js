...
function initScene(self, canvas, opts) {
    ...
    if(self.vrMode && !self.virtualJoystick && self.gyroEnabled && self.mobileMode) {
        if (navigator.getVRDisplays) {
            self._camera = new BABYLON.WebVRFreeCamera("camera", new vec3(0, 2, 0), scene);
        }
        else {
            self._camera = new BABYLON.VRDeviceOrientationFreeCamera("camera", new vec3(0, 2, 0), scene);
        }
